...
parish_data_raw <- 
  "data-raw" %>% 
  here("PJI_Data_Parish.Age.Charge.ConvictionDate.csv") %>% 
  fread(sep = ",", header = TRUE, stringsAsFactors = FALSE) %>% 
  clean_names() %>% 
  dplyr::select(-starts_with("time_spent")) %>% 
  mutate(conviction_date = as.Date(conviction_date, format = "%m/%d/%Y"))
...
parish_data_clean <- 
  ...
  mutate(charge = 
         ...
         rs_flag = 
           ...
           cumsum()) %>% 
  group_by(id, rs_flag, jdc, parish, age, conviction_date) %>% 
...
parish_data_wide_sentence_level <- 
  parish_data_clean %>% 
  group_by(id, sentence_id, jdc, parish, age, conviction_date) %>% 
...
parish_data_wide_full <- 
  ...
  group_by(id, jdc, parish, age, conviction_date) %>% 
  dplyr::summarize(across(c("charge", "state_crime_id", "charge_clean_full"), 
                          ~paste(., collapse = " / "))) %>% 
  ungroup() 
...
####Write Data####
##Sentence Level 
fwrite(parish_data_wide_sentence_level, here("data", "parish_data_wide_sentence_level.csv"), row.names = FALSE)

##Fully Wide 
fwrite(parish_data_wide_full, here("data", "parish_data_wide_full.csv"), row.names = FALSE)

##Tiydcensus LA Parish Data
fwrite(census_data_la, here("data", "census_data_la.csv"), row.names = FALSE)
